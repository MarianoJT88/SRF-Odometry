...
//		scene->insert( target );
        ...
        if (draw_srf_ma)
        {
            CSpherePtr keyscan = opengl::CSphere::Create(0.12, 20, 20);
            keyscan->setColor(0.f, 0.f, 1.f);
            keyscan->setLocation(robotpose3d[0], robotpose3d[1], 0.02f);
            scene->insert( keyscan );
        }

        //Extra viewport 1
        if (draw_vp1)
        {
            COpenGLViewportPtr vp1 = scene->createViewport("vp1");
            vp1->setViewportPosition(0.7,0.05,600,600);
            vp1->setCustomBackgroundColor(TColorf(1.f, 1.f, 1.f));

            //old scan
            CPointCloudPtr old_scan = opengl::CPointCloud::Create();
            old_scan->setColor(1.f, 0.f, 0.f);
            old_scan->setPointSize(3.f);
            vp1->insert(old_scan);

            //new scan
            CPointCloudPtr new_scan = opengl::CPointCloud::Create();
            new_scan->setColor(0.f, 0.f, 1.f);
            new_scan->setPointSize(3.f);
            vp1->insert(new_scan);
        }
...
//                traj_lines_csm->removeFirstLine();
        ...
        //Insert keyscan
        if (draw_srf_ma && odo_test.new_ref_scan)
        {
            CSpherePtr keyscan = scene->getByClass<CSphere>(0);
            keyscan->setLocation(odo_test.laser_pose[0], odo_test.laser_pose[1], 0.02f);
        }

        //Extra viewport 1
        if (draw_vp1)
        {
            COpenGLViewportPtr vp1 = scene->getViewport("vp1");

            //old scan
            CPointCloudPtr old_scan = vp1->getByClass<CPointCloud>(0);
            old_scan->clear();
            for (unsigned int u=0; u<odo.cols; u++)
                old_scan->insertPoint(odo.xx_old[repr_level](u), odo.yy_old[repr_level](u), 0.f);

            //new scan
            CPointCloudPtr new_scan = vp1->getByClass<CPointCloud>(1);
            new_scan->clear();
            for (unsigned int u=0; u<odo.cols; u++)
                new_scan->insertPoint(odo.xx[repr_level](u), odo.yy[repr_level](u), 0.f);
        }
